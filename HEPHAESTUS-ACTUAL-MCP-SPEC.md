...
*This replaces the REST API with proper MCP protocol*
...
- Over SSE (Server-Sent Events) or Streamable HTTP transport
- Specific schema for tools, resources, prompts
- What Claude connectors actually speak

## What We Built (Wrong)

REST API with endpoints like:
- `GET /health`
- `POST /tools/workflow/create`
- `GET /tools/file/read`

**This is NOT MCP. Claude connectors cannot use it.**
...
Create `/opt/leveredge/control-plane/agents/hephaestus/hephaestus_mcp_server.py`:
...
Create `/opt/leveredge/control-plane/agents/hephaestus/requirements.txt`:
...
```bash
# Stop old service
sudo systemctl stop hephaestus-mcp
...
# (hephaestus_mcp_server.py replaces hephaestus_mcp.py)

# Update service file to use new script
sudo nano /etc/systemd/system/hephaestus-mcp.service
# Change ExecStart to:
# ExecStart=/home/damon/.local/bin/uvicorn hephaestus_mcp_server:app --host 0.0.0.0 --port 8011

# Restart
sudo systemctl daemon-reload
sudo systemctl start hephaestus-mcp
```
...
2. Name: `HEPHAESTUS`
3. URL: `https://hephaestus.leveredgeai.com/sse`
4. Click Add
...
curl -I https://hephaestus.leveredgeai.com/sse

# Test health
curl https://hephaestus.leveredgeai.com/health
```
...
Once connected, I (Claude) will have these tools:

| Tool | What it does |
|------|--------------|
| `create_file` | Create files in /opt/leveredge/ |
| `read_file` | Read files |
| `list_directory` | List directory contents |
| `run_command` | Execute whitelisted commands |
| `list_workflows` | List n8n workflows |
| `create_workflow` | Create n8n workflows |
| `activate_workflow` | Activate workflows |
| `call_agent` | Call CHRONOS, HADES, AEGIS, etc. |
