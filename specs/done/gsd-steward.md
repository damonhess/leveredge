# GSD: STEWARD - Professional Coordination & Meeting Prep

**Priority:** HIGH
**Port:** 8220
**Domain:** THE KEEP
**Time:** ~15 min
**Purpose:** Coordinate between advisors, prep for professional meetings, track advice received

---

## THE VISION

STEWARD is your chief of staff for professional relationships. He:

- **Remembers** what your CPA, attorney, financial advisor said
- **Prepares** you for meetings (questions, docs, context)
- **Tracks** action items from professional advice
- **Coordinates** when questions span multiple domains (legal + tax)
- **Synthesizes** advice from SOLON, QUAESTOR, MIDAS into actionable plans

*"I serve the household. I remember what was said, prepare what is needed, and ensure nothing falls through the cracks."*
...
CREATE INDEX idx_prof_role ON steward_professionals(role);
CREATE INDEX idx_prof_domain ON steward_professionals(domain);
...
CREATE INDEX idx_meeting_prof ON steward_meetings(professional_id);
CREATE INDEX idx_meeting_date ON steward_meetings(meeting_date);
CREATE INDEX idx_meeting_status ON steward_meetings(status);
...
CREATE INDEX idx_advice_prof ON steward_advice(professional_id);
CREATE INDEX idx_advice_topic ON steward_advice(topic);
CREATE INDEX idx_advice_category ON steward_advice(category);
CREATE INDEX idx_advice_action ON steward_advice(requires_action, action_status);
...
CREATE INDEX idx_questions_status ON steward_questions(status);
CREATE INDEX idx_questions_role ON steward_questions(target_role);
...
CREATE INDEX idx_docs_type ON steward_documents(document_type);
CREATE INDEX idx_docs_status ON steward_documents(status);
```
...
async def health():
    return {
        "status": "healthy",
        "service": "STEWARD - Professional Coordination",
        "port": 8220,
...
async def complete_meeting(
    ...
    async with pool.acquire() as conn:
        await conn.execute("""
            ...
            WHERE id = $1::uuid
        """, meeting_id, summary, json.dumps(advice_received),
            json.dumps(action_items), follow_up_date)
...
async def add_document(
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow("""
            ...
            VALUES ($1, $2, $3, $4, $5)
            RETURNING *
        """, name, document_type, year, due_date, status)
        return dict(row)
...
CMD ["uvicorn", "steward:app", "--host", "0.0.0.0", "--port", "8220"]
```
...
```yaml
  steward:
    ...
    healthcheck:
      ...
      retries: 3
    labels:
      - "leveredge.agent=STEWARD"
      - "leveredge.domain=THE_KEEP"
      - "leveredge.port=8220"
```
...
- [ ] Database schema (professionals, meetings, advice, questions, documents)
- [ ] Full FastAPI implementation
- [ ] AI-powered meeting prep
- [ ] Advice synthesis across professionals + agents
...
```bash
# Add a professional
curl -X POST http://localhost:8220/professionals \
  -H "Content-Type: application/json" \
  -d '{"name": "Jane Smith", "role": "cpa", "company": "Smith Tax LLC", "domain": "both"}'

# Schedule a meeting
curl -X POST http://localhost:8220/meetings \
  -H "Content-Type: application/json" \
  -d '{"professional_id": "...", "title": "Q1 Tax Planning", "meeting_date": "2026-02-01T10:00:00Z", "topics": ["s_corp", "deductions"]}'

# Generate meeting prep
curl -X POST http://localhost:8220/meetings/{id}/prep

# Log a question to ask
curl -X POST http://localhost:8220/questions \
  -H "Content-Type: application/json" \
  -d '{"question": "Should I elect S-Corp for LeverEdge?", "target_role": "cpa", "domain": "business"}'

# Synthesize advice on a topic
curl -X POST "http://localhost:8220/synthesize?topic=entity_structure&domain=business"
```
...
PROFESSIONALS:
- Track CPAs, attorneys, financial advisors
- Store contact info, specialty, last/next meetings

MEETINGS:
- Schedule and track professional meetings
- AI-powered meeting prep (questions, docs, context)
- Record outcomes, advice, action items

ADVICE:
- Log all advice received from professionals
- Track action items with deadlines
- Synthesize across multiple sources

DOCUMENTS:
- Track needed documents for professionals
- Due dates and status tracking

I serve the household. I remember, prepare, and coordinate.
```
