...
Usage:
    from shared.cost_tracker import CostTracker, log_llm_usage

    tracker = CostTracker("SOLON")
...
Location: /opt/leveredge/shared/cost_tracker.py
"""
...
FEATURE_COSTS = {
    ...
    "audio_transcription": 0.006,  # per minute (Whisper)
}
...
class UsageRecord:
    ...
    output_tokens: int
    web_searches: int = 0
    other_features: Dict[str, int] = field(default_factory=dict)
...
class CostTracker:
    ...
    def __init__(self, agent_name: str):
        ...
        self.enabled = os.environ.get("COST_TRACKING_ENABLED", "true").lower() == "true"
    ...
    async def log_usage(
        ...
        Args:
            endpoint: API endpoint that incurred the cost
            model: LLM model used
            input_tokens: Number of input tokens
            output_tokens: Number of output tokens
            web_searches: Number of web searches performed
            other_features: Dict of feature_name -> count for other billable features
            metadata: Additional context to store with the record
    ...
    async def log_external_api(
        ...
        """
        Log external API usage (non-LLM services).

        Args:
            service: Name of external service (e.g., "courtlistener", "sendgrid")
            endpoint: API endpoint called
            cost: Cost incurred (if known, else 0)
    ...
    async def _log_to_database(self, entry: dict) -> None:
        ...
        if not SUPABASE_KEY:
            logger.debug(f"[CostTracker] No Supabase key, skipping DB log")
            return
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code not in [200, 201]:
                    logger.warning(f"[CostTracker] DB log failed: {response.status_code}")
        except Exception as e:
            logger.warning(f"[CostTracker] DB log error: {e}")
    ...
    async def _log_to_event_bus(self, entry: dict) -> None:
        ...
        try:
            ...
            logger.debug(f"[CostTracker] Event bus error: {e}")
...
async def log_llm_usage(
    ...
    Args:
        ...
        web_searches: Number of web searches (auto-detected if not provided)
    ...
    # Auto-detect web searches if not provided
    if web_searches == 0:
        web_searches = tracker.count_web_searches(response)
