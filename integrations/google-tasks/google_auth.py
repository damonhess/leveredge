"""
Google OAuth2 Authentication Handler for Tasks API
Manages credentials, token refresh, and OAuth flow
Shared pattern with google-calendar integration
"""
...
# OAuth2 scopes required for Tasks access
SCOPES = [
...
DEFAULT_CREDENTIALS_PATH = Path('/opt/leveredge/integrations/google-tasks/credentials')
...
_auth_manager: Optional[GoogleTasksAuthManager] = None


def get_auth_manager() -> GoogleTasksAuthManager:
    """Get or create the singleton auth manager instance."""
    global _auth_manager
    if _auth_manager is None:
        _auth_manager = GoogleTasksAuthManager()
    return _auth_manager


def get_tasks_service() -> Resource:
    """Convenience function to get Tasks API service."""
    return get_auth_manager().get_tasks_service()
